...
import { TrendingUp, ArrowRight, Truck, Clock, AlertTriangle, MapPin, Phone, Star, Hand } from 'lucide-react'
...
function StatCard({ stat }) {
  return (
    <div className={`bg-(--bg) rounded-2xl border p-5 flex flex-col gap-3 ${stat.trend==='alert'?'border-clay-300 dark:border-clay-700':'border-(--border)'}`}>
      ...
      <p className={`text-[12.5px] flex items-center gap-1 ${stat.trend==='up'?'text-green-600 dark:text-green-400':stat.trend==='alert'?'text-clay-500':'text-(--text-muted)'}`}>
      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-7">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[14px] text-(--text-muted) mt-1">{today} · {logisticsProfile.name}</p>
          </div>
          <Button as={Link} to="/logistics/requests" variant="accent" size="md"><Truck size={16}/>View requests</Button>
        </div>

        {/* Vehicle info strip */}
        <div className="flex flex-wrap items-center gap-6 p-4 rounded-2xl bg-(--bg) border border-(--border)">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[14px] font-medium text-(--text)">{logisticsProfile.vehicle}</p>
              <p className="text-[12.5px] text-(--text-muted)">Plate: {logisticsProfile.plate}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 text-[13.5px] text-(--text-muted)"><MapPin size={14}/>{logisticsProfile.coverage}</div>
          <div className="flex items-center gap-1.5 text-[13.5px] text-(--text-muted)"><TrendingUp size={14}/>₦{logisticsProfile.pricePerKm}/km base rate</div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-medium text-[15px] text-(--text)">Monthly earnings</h3>
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-(--border)">
                <h3 className="font-medium text-[15px] text-(--text)">Pending requests</h3>
                ...
                {deliveryRequests.slice(0,3).map(r=>(
                  <div key={r.id} className="px-6 py-4 flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[14.5px] font-medium text-(--text)">{r.farmer}</p>
                      <p className="text-[13px] text-(--text-muted) mt-0.5">{r.produce} · {r.qty} · {r.distance}</p>
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="flex items-center gap-1 text-[12px] text-(--text-muted)"><Clock size={11}/>{r.pickupDate}</span>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      ...
                      <a href={`tel:${r.phone}`} className="px-3 py-1.5 rounded-xl border border-(--border) text-(--text-muted) text-[12.5px] text-center hover:bg-(--bg-subtle) transition-colors flex items-center justify-center gap-1"><Phone size={12}/>Call</a>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-(--border)">
                <h3 className="font-medium text-[15px] text-(--text)">Active deliveries</h3>
                ...
                {activeDeliveries.map(d=>(
                  <div key={d.id} className="px-5 py-4">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div>
                        <p className="text-[14px] font-medium text-(--text)">{d.produce}</p>
                        <p className="text-[12.5px] text-(--text-muted) mt-0.5">{d.farmer} · {d.qty}</p>
                      </div>
                    ...
                    <div className="flex items-center gap-2 text-[12px] text-(--text-muted)">
                      ...
                      <span className="ml-auto font-mono font-medium text-[12.5px] text-(--text)">{fmt(d.fee)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
              <div className="px-5 py-4 border-b border-(--border)">
                <h3 className="font-medium text-[15px] text-(--text)">Recent completed</h3>
              </div>
              <div className="divide-y divide-(--border)">
                {completedDeliveries.slice(0,3).map(d=>(
                  <div key={d.id} className="px-5 py-3.5 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[13.5px] font-medium text-(--text)">{d.produce}</p>
                      <p className="text-[12px] text-(--text-muted) mt-0.5">{d.from} → {d.to} · {d.date}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <span className="font-mono text-[13.5px] font-medium text-(--text)">{fmt(d.fee)}</span>
