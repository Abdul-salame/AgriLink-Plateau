...
export default function MyVehicle() {
  ...
  return (
    <LogisticsLayout title="My Vehicle">
      <div className="px-6 lg:px-8 py-8 max-w-150 mx-auto">
        <div className="mb-7">
          <h1 className="font-display text-[26px] font-medium text-navy-700 dark:text-navy-100">My vehicle</h1>
          <p className="text-[14px] text-(--text-muted) mt-1">Keep your vehicle details accurate so farmers can make informed booking decisions.</p>
        </div>
        {saved && (
        ...
        <div className="bg-(--bg) rounded-2xl border border-(--border) p-6 flex flex-col gap-5">
          <div className="flex items-center gap-4 pb-2 border-b border-(--border)">
            ...
            <div>
              <p className="text-[15px] font-medium text-(--text)">{fields.vehicleType}</p>
              <p className="text-[13px] text-(--text-muted)">Plate: {fields.plate}</p>
            </div>
            <span className={`ml-auto text-[12px] font-medium px-3 py-1.5 rounded-full border ${fields.available?'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border-green-200 dark:border-green-800':'bg-(--bg-subtle) text-(--text-muted) border-(--border)'}`}>
          ...
          <div className="flex items-center justify-between py-2">
            <div>
              <p className="text-[14px] font-medium text-(--text)">Available for pickups</p>
              <p className="text-[13px] text-(--text-muted) mt-0.5">Toggle off when you're on a delivery or unavailable</p>
            </div>
            <button onClick={()=>setFields(f=>({...f,available:!f.available}))}
              className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${fields.available?'bg-navy-600':'bg-(--border-mid)'}`}>
