...
import { AlertTriangle, CheckCircle2, ChevronDown, ChevronUp, Phone } from 'lucide-react'
...
function DisputeCard({ dispute, onResolve }) {
  ...
  return (
    <div className={`bg-[var(--bg)] rounded-2xl border overflow-hidden ${dispute.status === 'open' && dispute.priority === 'high' ? 'border-red-200 dark:border-red-800' : 'border-[var(--border)]'}`}>
      <div className="flex items-start gap-4 p-5 cursor-pointer" onClick={() => setExpanded(v => !v)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <span className="font-mono text-[12.5px] text-[var(--text-muted)]">{dispute.id}</span>
            <span className="font-mono text-[12px] text-[var(--text-muted)]">→ {dispute.order}</span>
          ...
          <p className="text-[14.5px] font-medium text-[var(--text)]">{dispute.buyer} vs {dispute.farmer}</p>
          <p className="text-[13px] text-[var(--text-muted)] mt-0.5 line-clamp-1">{dispute.issue}</p>
        </div>
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          ...
          <span className="text-[12px] text-[var(--text-muted)]">{dispute.date}</span>
        </div>
      ...
      {expanded && (
        <div className="border-t border-[var(--border)] p-5 flex flex-col gap-4 bg-[var(--bg-subtle)]">
          <div>
            <p className="text-[13px] font-medium text-[var(--text-muted)] mb-1.5">Issue reported</p>
            <p className="text-[14px] text-[var(--text)] leading-relaxed">{dispute.issue}</p>
          </div>
          <div className="grid sm:grid-cols-2 gap-4 text-[13.5px]">
            <div className="p-3.5 rounded-xl bg-[var(--bg)] border border-[var(--border)]">
              <p className="text-[var(--text-muted)] text-[12.5px] mb-1">Buyer</p>
              <p className="font-medium text-[var(--text)]">{dispute.buyer}</p>
            </div>
            <div className="p-3.5 rounded-xl bg-[var(--bg)] border border-[var(--border)]">
              <p className="text-[var(--text-muted)] text-[12.5px] mb-1">Farmer</p>
              <p className="font-medium text-[var(--text)]">{dispute.farmer}</p>
            </div>
          </div>
          ...
          {dispute.status === 'open' && (
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="text-[13.5px] font-medium text-[var(--text)]">Resolution note</label>
    ...
    <AdminLayout title="Disputes">
      <div className="px-6 lg:px-8 py-8 max-w-[900px] mx-auto">
        <div className="mb-7">
          ...
          <p className="text-[14px] text-[var(--text-muted)] mt-1">{openCount} open · {resolvedCount} resolved</p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-[var(--bg)] border border-[var(--border)] rounded-xl p-1 mb-5 self-start w-fit">
          {[['all','All'],['open','Open'],['resolved','Resolved']].map(([v,l]) => (
            <button key={v} onClick={() => setFilter(v)}
              className={`px-4 py-1.5 rounded-lg text-[13px] font-medium transition-all ${filter===v ? 'bg-navy-600 text-white' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}>{l}</button>
          ))}
        </div>

        <div className="flex flex-col gap-3">
          {filtered.length === 0
            ? <div className="text-center py-16 text-[var(--text-muted)]">No disputes found.</div>
