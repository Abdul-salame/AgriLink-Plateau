...
import { TrendingUp, ArrowRight, AlertTriangle, UserCheck, ShoppingCart, Users, Hand } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  return (
    <AdminLayout title="Dashboard">
      <div className="px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-8 max-w-7xl mx-auto space-y-5 sm:space-y-6 lg:space-y-7">

        {/* Greeting */}
        <div>
          <h2 className="font-display text-[22px] sm:text-[26px] font-medium text-navy-700 dark:text-navy-100 flex items-center gap-2">
            <span>Good morning, {adminProfile.name.split(' ')[0]}</span>
            <Hand size={22} className="text-clay-500" />
          </h2>
          <p className="text-[14px] text-(--text-muted) mt-1">{today} · Platform overview</p>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <div className="rounded-2xl border border-(--border) bg-(--bg) p-4 sm:p-6">
            <div className="mb-6 flex items-center justify-between gap-3">
              <h3 className="font-medium text-[15px] text-(--text)">User growth</h3>
          ...
          <div className="rounded-2xl border border-(--border) bg-(--bg) p-4 sm:p-6">
            <div className="mb-6 flex items-center justify-between gap-3">
              <h3 className="font-medium text-[15px] text-(--text)">Monthly orders</h3>
              <span className="text-[13px] text-(--text-muted)">Last 6 months</span>
            </div>
            <ResponsiveContainer width="100%" height={220}>
        ...
        <div className="overflow-hidden rounded-2xl border border-(--border) bg-(--bg)">
          <div className="flex flex-col gap-2 border-b border-(--border) px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <h3 className="font-medium text-[15px] text-(--text)">Recent activity</h3>
            <span className="text-[13px] text-(--text-muted)">Live feed</span>
          </div>
          <div className="divide-y divide-(--border)">
            {recentActivity.map(a => {
              const Icon = ACTIVITY_ICONS[a.type] ?? AlertTriangle
              return (
                <div key={a.id} className="flex flex-col gap-3 px-4 py-4 hover:bg-(--bg-subtle) transition-colors sm:flex-row sm:items-start sm:gap-4 sm:px-6">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${ACTIVITY_STYLE[a.status]}`}>
                    <Icon size={14} strokeWidth={2} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] text-(--text)">{a.message}</p>
                  </div>
                  <span className="text-[12px] text-(--text-muted) shrink-0">{a.time}</span>
                </div>
              )
            })}
          </div>
