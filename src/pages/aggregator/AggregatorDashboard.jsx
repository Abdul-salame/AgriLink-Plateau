      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-7">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[14px] text-(--text-muted) mt-1">{today} · {aggregatorProfile.businessName}</p>
          </div>
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead><tr className="bg-(--bg-subtle) text-[12.5px] text-(--text-muted)">
                  ...
                  <tbody>
                    {aggregatorOrders.map((o,i)=>(
                      <tr key={o.id} className={`hover:bg-(--bg-subtle) transition-colors ${i<aggregatorOrders.length-1?'border-b border-(--border)':''}`}>
                        ...
                        <td className="px-6 py-3.5 text-[14px] text-(--text-muted) hidden sm:table-cell">{o.produce}</td>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-(--border)">
                <h3 className="font-medium text-[15px] text-(--text)">Farmer network</h3>
                ...
                {farmerNetwork.slice(0,4).map(f=>(
                  <div key={f.id} className="flex items-center gap-3 px-5 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] font-medium text-(--text) truncate">{f.name}</p>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <span className="text-[11px] text-(--text-muted)">{f.commission}% comm.</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-(--border)">
                <h3 className="font-medium text-[15px] text-(--text)">Bulk listings</h3>
              ...
              <div className="divide-y divide-(--border)">
                {bulkListings.filter(l=>l.status==='active').map(l=>(
                  <div key={l.id} className="px-5 py-3.5 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[14px] font-medium text-(--text)">{l.produce}</p>
                      <div className="flex gap-3 mt-0.5">
                        <span className="text-[12px] text-(--text-muted) flex items-center gap-1"><Eye size={11}/>{l.views}</span>
                        <span className="text-[12px] text-(--text-muted) flex items-center gap-1"><Tag size={11}/>{fmt(l.price)}/{l.unit}</span>
                      </div>
                    </div>
