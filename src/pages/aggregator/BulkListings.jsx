...
export default function BulkListings() {
  const [listings, setListings] = useState(bulkListings)
      ...
      <div className="px-6 lg:px-8 py-8 max-w-[1000px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-7">
          <div>
            ...
            <p className="text-[14px] text-[var(--text-muted)] mt-1">{listings.filter(l=>l.status==='active').length} active · {listings.filter(l=>l.status==='sold').length} sold</p>
          </div>
        ...
        <div className="flex items-center gap-1 bg-[var(--bg)] border border-[var(--border)] rounded-xl p-1 mb-6 w-fit">
          {['all','active','sold'].map(s=>(
            <button key={s} onClick={()=>setFilter(s)}
              className={`px-4 py-1.5 rounded-lg text-[13px] font-medium capitalize transition-all ${filter===s?'bg-navy-600 text-white':'text-[var(--text-muted)] hover:text-[var(--text)]'}`}>{s}</button>
          ))}
        </div>
          ...
          {filtered.map(l=>(
            <div key={l.id} className="bg-[var(--bg)] rounded-2xl border border-[var(--border)] p-5 flex flex-col gap-4 hover:border-[var(--border-mid)] transition-colors">
              ...
              <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-[13px]">
                ...
                <span className="flex items-center gap-1.5 text-[var(--text-muted)]"><Eye size={13}/>{l.views} views</span>
                <span className="flex items-center gap-1.5 text-[var(--text-muted)]"><Users size={13}/>{l.farmers} farmer{l.farmers!==1?'s':''}</span>
                <span className="flex items-center gap-1.5 text-[var(--text-muted)]"><Calendar size={13}/>Harvest {l.harvestDate}</span>
              </div>
              {l.offers>0 && (
