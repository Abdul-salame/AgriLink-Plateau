...
function ProduceCard({ item, saved, onSave, onOrder }) {
  const farmerName = item.farmer?.firstName ? `${item.farmer.firstName} ${item.farmer.lastName}` : 'Farmer'
  const verified = item.farmer?.kyc?.status === 'approved'
  ...
  return (
    <div className="bg-(--bg) rounded-2xl border border-(--border) flex flex-col overflow-hidden hover:border-(--border-mid) hover:shadow-sm transition-all">
      <div className="h-36 bg-navy-50 dark:bg-navy-800 relative overflow-hidden">
        ...
        <button onClick={() => onSave(item._id)}
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        <div>
          ...
          <p className="text-[12.5px] text-(--text-muted) mt-0.5">{farmerName} · {item.lga}</p>
          <div className="flex items-center gap-1 mt-1">
            <Star size={12} className="text-gold-400 fill-gold-400" />
            <span className="text-[12px] text-(--text-muted)">{item.farmer?.rating ?? 0} ({item.farmer?.totalReviews ?? 0} reviews)</span>
          </div>
        </div>
        ...
        <div className="mt-auto pt-1">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[12px] text-(--text-muted)">{item.quantity} available</span>
          </div>
...
function OrderModal({ item, onClose, onPlaced }) {
  ...
  if (done) return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-(--bg) rounded-2xl p-8 max-w-sm w-full text-center">
        ...
        <p className="text-[14px] text-(--text-muted) mb-6">Your order has been sent to {item.farmer?.firstName ? `${item.farmer.firstName} ${item.farmer.lastName}` : 'the farmer'}. They will confirm within 24 hours.</p>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-(--bg) rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 rounded-xl bg-(--bg-subtle) border border-(--border) mb-5">
          ...
          <p className="text-[13px] text-(--text-muted) mt-0.5">{item.farmer?.firstName ? `${item.farmer.firstName} ${item.farmer.lastName}` : 'Farmer'} · {item.lga}</p>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            }} disabled={submitting}>{submitting ? 'Placing order…' : 'Confirm order'}</Button>
...
export default function BrowseProduce() {
    ...
    async function loadListings() {
      ...
      try {
        const { data } = await api.get('/listings', {
          params: { verified: verifiedOnly ? 'true' : undefined, search, category: category === 'All' ? undefined : category, lga: lga === 'All LGAs' ? undefined : lga, sort: sortBy === 'price' ? 'price_asc' : sortBy === 'price_desc' ? 'price_desc' : 'rating' },
        })
        if (mounted) setListings(data.data || [])
      } catch (err) {
        if (mounted) setError(err.response?.data?.message || 'Unable to load listings right now.')
      } finally {
  ...
  }, [search, category, lga, verifiedOnly, sortBy])
  ...
  const filtered = listings
    .filter(p => {
      const farmerName = `${p.farmer?.firstName || ''} ${p.farmer?.lastName || ''}`.trim()
      const matchSearch   = p.produce.toLowerCase().includes(search.toLowerCase()) || farmerName.toLowerCase().includes(search.toLowerCase())
      return matchSearch
    })
    .sort((a, b) => sortBy === 'price' ? a.price - b.price : sortBy === 'price_desc' ? b.price - a.price : (b.farmer?.rating || 0) - (a.farmer?.rating || 0))
  ...
  return (
    <BuyerLayout title="Browse Produce">
      <div className="px-6 lg:px-8 py-8 max-w-300 mx-auto">
        ...
        {loading ? (
          <div className="text-center py-20 text-(--text-muted)">Loading produce…</div>
        ) : filtered.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map(p => (
              <ProduceCard key={p._id} item={p} saved={savedIds.has(p._id)} onSave={toggleSave} onOrder={setOrderItem} />
            ))}
          </div>
        )}
      </div>
      {orderItem && <OrderModal item={orderItem} onClose={() => setOrderItem(null)} onPlaced={() => setOrderItem(null)} />}
    </BuyerLayout>
  )
}