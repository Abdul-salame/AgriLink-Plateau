...
import { BookmarkX, Calendar, Package, Tag, ShoppingBasket } from 'lucide-react'
    ...
    <BuyerLayout title="Saved Listings">
      <div className="px-6 lg:px-8 py-8 max-w-[900px] mx-auto">
        <div className="mb-7">
          ...
          <p className="text-[14px] text-[var(--text-muted)] mt-1">{saved.length} saved listing{saved.length !== 1 ? 's' : ''}</p>
        </div>
        ...
        {saved.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-[15px] text-[var(--text-muted)]">No saved listings yet.</p>
            <p className="text-[13.5px] text-[var(--text-muted)] mt-1">Bookmark produce while browsing to save them here.</p>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[12.5px] text-[var(--text-muted)]">{l.farmer} · {l.lga}</p>
                    </div>
                  </div>
                  <button onClick={() => remove(l.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-[var(--text-muted)] hover:text-red-500 transition-colors shrink-0">
                    <BookmarkX size={16} />
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-[13px]">
                  <span className="flex items-center gap-1.5 text-[var(--text-muted)]"><Package size={13} />{l.qty}</span>
                  <span className="flex items-center gap-1.5 text-[var(--text-muted)]"><Tag size={13} />{fmt(l.price)}/{l.unit}</span>
                  <span className="flex items-center gap-1.5 text-[var(--text-muted)] col-span-2"><Calendar size={13} />Harvest {l.harvestDate}</span>
                </div>
                {l.verified && (
