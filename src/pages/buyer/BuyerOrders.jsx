    ...
    <div className="bg-(--bg) rounded-2xl border border-(--border) overflow-hidden">
      <div className="flex items-start justify-between gap-4 p-5 cursor-pointer" onClick={() => setExpanded(v => !v)}>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[15px] font-medium text-(--text)">{order.produce}</p>
          <p className="text-[13px] text-(--text-muted) mt-0.5">{order.farmer} · {order.farmerLga} · {order.qty}</p>
        </div>
        ...
        <div className="text-(--text-muted) shrink-0 mt-1">{expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
      </div>
      ...
      {expanded && (
        <div className="border-t border-(--border) p-5 flex flex-col gap-4 bg-(--bg-subtle)">
          <div className="grid sm:grid-cols-2 gap-4 text-[13.5px]">
            <div><p className="text-(--text-muted)">Expected delivery</p><p className="font-medium text-(--text) mt-0.5">{order.deliveryDate}</p></div>
            <div><p className="text-(--text-muted)">Farmer contact</p>
          ...
          {order.note && (
            <div className="p-3 rounded-xl bg-(--bg) border border-(--border) text-[13.5px]">
              <span className="text-(--text-muted)">Your note: </span><span className="text-(--text)">{order.note}</span>
            </div>
          )}
          {/* Progress */}
          <div>
              ...
              {STATUS_FLOW.map((s, i) => {
                ...
                return (
                  <div key={s} className="flex items-center flex-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${done ? 'bg-navy-600' : 'bg-(--border-mid)'}`}>
                      {done ? <CheckCircle2 size={13} className="text-white" /> : <span className="w-2 h-2 rounded-full bg-white/50" />}
                    </div>
                    {i < STATUS_FLOW.length - 1 && <div className={`flex-1 h-1 ${i < currentStep ? 'bg-navy-600' : 'bg-(--border-mid)'}`} />}
                  </div>
                )
              })}
            </div>
            <div className="flex justify-between mt-2">
              {STATUS_FLOW.map(s => <span key={s} className="text-[10.5px] capitalize text-(--text-muted)">{STATUS_META[s].label}</span>)}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="px-6 lg:px-8 py-8 max-w-225 mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-7">
          <div>
            ...
            <p className="text-[14px] text-(--text-muted) mt-0.5">Total spent on delivered orders: {fmt(totalSpent)}</p>
          </div>
        </div>
