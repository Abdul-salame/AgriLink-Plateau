...
import { LayoutDashboard, Inbox, Truck, Wallet, Car, Settings, LogOut, Sprout, ChevronRight } from 'lucide-react'
import { logisticsProfile } from '../data/logisticsData'

const NAV = [
  { label: 'Dashboard',          icon: LayoutDashboard, path: '/logistics/dashboard'  },
  { label: 'Delivery Requests',  icon: Inbox,           path: '/logistics/requests', badge: 5 },
  { label: 'My Deliveries',      icon: Truck,           path: '/logistics/deliveries' },
  { label: 'Earnings',           icon: Wallet,          path: '/logistics/earnings'   },
  { label: 'My Vehicle',         icon: Car,             path: '/logistics/vehicle'    },
  { label: 'Settings',           icon: Settings,        path: '/logistics/settings'   },
]
...
export default function LogisticsSidebar({ mobileOpen, setMobileOpen }) {
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col bg-navy-900 border-r border-navy-800 transition-all duration-300 lg:relative lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${collapsed ? 'w-[68px]' : 'w-[240px]'}`}>
        <div className={`flex items-center h-16 border-b border-navy-800 px-4 shrink-0 ${collapsed ? 'justify-center' : 'gap-3'}`}>
          ...
          {!collapsed && <div><p className="font-display text-[16px] font-medium text-white leading-tight">AgriLink</p><p className="text-[11px] text-orange-400 font-medium uppercase tracking-wide">Logistics</p></div>}
        </div>
        ...
        {!collapsed && (
          <div className="px-3 py-3 mx-2 mb-2 rounded-xl bg-navy-800 border border-navy-700">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center text-white font-display font-medium text-[14px] shrink-0">{logisticsProfile.avatar}</div>
              <div className="min-w-0 flex-1">
                <p className="text-[13px] font-medium text-white truncate">{logisticsProfile.contactName}</p>
                <p className="text-[11.5px] text-navy-400 truncate">Logistics</p>
              </div>
