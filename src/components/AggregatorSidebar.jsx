...
import { LayoutDashboard, Users, ListChecks, ShoppingCart, TrendingUp, Wallet, Settings, LogOut, Sprout, ChevronRight } from 'lucide-react'
import { aggregatorProfile } from '../data/aggregatorData'

const NAV = [
  { label: 'Dashboard',       icon: LayoutDashboard, path: '/aggregator/dashboard' },
  { label: 'Farmer Network',  icon: Users,           path: '/aggregator/farmers'   },
  { label: 'Bulk Listings',   icon: ListChecks,      path: '/aggregator/listings'  },
  { label: 'Orders',          icon: ShoppingCart,    path: '/aggregator/orders', badge: 2 },
  { label: 'Market Prices',   icon: TrendingUp,      path: '/aggregator/prices'    },
  { label: 'Payments',        icon: Wallet,          path: '/aggregator/payments'  },
  { label: 'Settings',        icon: Settings,        path: '/aggregator/settings'  },
]
...
export default function AggregatorSidebar({ mobileOpen, setMobileOpen }) {
        ...
        {!collapsed && (
          <div className="px-3 py-3 mx-2 mb-2 rounded-xl bg-navy-800 border border-navy-700">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-purple-500 flex items-center justify-center text-white font-display font-medium text-[14px] shrink-0">{aggregatorProfile.avatar}</div>
              <div className="min-w-0 flex-1">
                <p className="text-[13px] font-medium text-white truncate">{aggregatorProfile.firstName}</p>
                <p className="text-[11.5px] text-navy-400 truncate">Aggregator</p>
              </div>
