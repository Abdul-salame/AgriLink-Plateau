...
export default function ExtensionLayout({ children, title }) {
  const [mobileOpen, setMobileOpen] = useState(false)
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--bg-subtle)]">
      <ExtensionSidebar mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-16 bg-[var(--bg)] border-b border-[var(--border)] flex items-center gap-4 px-6 shrink-0">
          ...
          <div className="w-9 h-9 rounded-full bg-teal-500 flex items-center justify-center text-white font-display font-medium text-[14px]">{extensionProfile.avatar}</div>
        </header>
